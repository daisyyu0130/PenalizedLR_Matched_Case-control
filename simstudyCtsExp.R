...
# beta and Rsquared.
params <- expand.grid(nmatch,betas,ncov)
names(params) <- c("nmatch","beta","ncov")
params <- data.frame(params, exptype=rep("continuous",nrow(params)))
head(params)
# 3. Other parameters 
NREPS <- 1000; conf.level <- 0.95; test.level <- 0.05; maxiter <- 500
